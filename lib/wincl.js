...
// delta is +1 / -1 in a stable top-to-bottom, left-to-right rotation.
const moveToMonitor = (cache, hwnd, delta) => run(cache, ['movemon', String(hwnd), String(delta)])[0] || null;

function monitors(cache) {
  const exe = ensureBuilt(cache);
  let out = '';
  try { out = execFileSync(exe, ['monitors'], { cwd: '/mnt/c', encoding: 'utf8', stdio: ['ignore','pipe','pipe'] }); } catch (e) { out = e.stdout || ''; }
  return out.split('\n').map((l) => l.replace(/\r$/, '').split('\t'))
    .filter((f) => f.length >= 5)
    .map((f, i) => ({ index: i, x: +f[0], y: +f[1], w: +f[2], h: +f[3], primary: f[4] === 'primary',
                      wx: +f[5], wy: +f[6], ww: +f[7], wh: +f[8] }));
}
...
module.exports = { list, foreground, focus, match, moveToMonitor, monitors };
